...
float kalmanYaw = 0.0;
float kalmanX = 0.0, kalmanY = 0.0;
const float Q_angle = 0.001;    // IMU noise
const float Q_bias = 0.003;     // Bias noise
const float R_measure = 0.03;   // GPS noise
float P[2][2] = {{0,0}, {0,0}};
...
File mapBestand;
// ADD AFTER GLOBAL VARIABLES (line ~85, before setup())
float lastValidX = 0.0, lastValidY = 0.0;  // GPS filter globals
...
void loop() {
  ...
  // 🔥 KALMAN FUSION
  kalmanUpdateYaw(yaw, gps.course.deg());
  kalmanUpdatePosition(posX, posY, gps.location.lat(), gps.location.lng());
...
// REPLACE your gpsInlezen() function (around line 170)
void gpsInlezen() {
  while (neogps.available()) gps.encode(neogps.read());
  
  if (gps.location.isValid()) {
    float gpsMetersX, gpsMetersY;
    gpsToMeters(gps.location.lat(), gps.location.lng(), gpsMetersX, gpsMetersY);
    
    if (gpsFilter(gpsMetersX, gpsMetersY)) {
      Serial.printf("✅ GPS: %.1fm, %.1fm\n", gpsMetersX, gpsMetersY);
    }
  }
}
...
void imuInlezen() {
  ...
  yaw -= 17.9;  // 🔥 PARAMARIBO 2026 DECLINATION
  if (yaw < 0) yaw += 360;
}


void kalmanUpdateYaw(float imuYaw, float gpsCourse) {
  // Predict
  kalmanYaw += 0.03 * (imuYaw - kalmanYaw);
  
  // GPS correction (if valid)
  if (gps.course.isValid()) {
    float gpsYaw = gps.course.deg();
    float error = gpsYaw - kalmanYaw;
...
void kalmanUpdatePosition(float imuX, float imuY, float gpsLat, float gpsLng) {
  // IMU dead reckoning prediction
  kalmanX += 0.02 * (imuX - kalmanX);
  kalmanY += 0.02 * (imuY - kalmanY);
  
  // GPS correction (if valid)
  if (gps.location.isValid()) {
    float gpsX = gps.location.lat() * 111000;  // Meters
    float gpsY = gps.location.lng() * 109600;  // Suriname meters
    kalmanX += 0.1 * (gpsX - kalmanX);
    kalmanY += 0.1 * (gpsY - kalmanY);
  }
}

// ADD THIS LATER - Autonomous straight line
void autonomousStraight(float targetX, float targetY) {
  float targetBearing = calculateBearing(kalmanX, kalmanY, targetX, targetY);
  float error = targetBearing - kalmanYaw;
  if (error > 180) error -= 360;
  
  int leftSpeed = 50 + error * 2;
  int rightSpeed = 50 - error * 2;
  rotateMotor(rightSpeed, leftSpeed);
}
...
// NEW FUNCTION 1 - GPS → Meters (Suriname optimized)
void gpsToMeters(float lat, float lng, float &metersX, float &metersY) {
  metersX = lat * 111000;        // N-S meters
  metersY = lng * 109600;        // E-W Suriname meters (cos(5° lat))
}
// NEW FUNCTION 2 - Calculate bearing to target (from your app)
float calculateBearing(float fromX, float fromY, float toX, float toY) {
  float deltaX = toX - fromX;
  float deltaY = toY - fromY;
  float bearing = atan2(deltaY, deltaX) * 180.0 / PI;
  if (bearing < 0) bearing += 360;
  return bearing;
}
// NEW FUNCTION 3 - 1m filter (like your React Native app)
bool gpsFilter(float newX, float newY) {
  float distance = sqrt(sq(newX - lastValidX) + sq(newY - lastValidY));
  if (distance >= 1.0) {  // Real 1m+ movement
    lastValidX = newX;
    lastValidY = newY;
    return true;
  }
...
void logPositie() {
  if (millis() - laatsteLogtijd > 1000) {
    if (mapBestand) {
      mapBestand.print(kalmanX, 2); mapBestand.print(",");
      mapBestand.print(kalmanY, 2); mapBestand.print(",");
      mapBestand.print(kalmanYaw, 1); mapBestand.print(",");
...
void handleClient() {
  ...
  String req = client.readStringUntil('\r');  // Remove \\r → '\r'
